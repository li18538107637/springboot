...
@ServerEndpoint(value = "/chat/{ip}")
@Component
@Scope("prototype")
public class WebSocketService implements BaseJob {
    ...
    public static void sendMessageAll(final String message){
        ...
        Set<Map.Entry<String, WebSocketService>> entries = webSocketMap.entrySet();
        for (Map.Entry<String, WebSocketService> entry : entries) {
            final WebSocketService webSocketServer = entry.getValue();
    ...
    public static void setWebSocketMap(ConcurrentHashMap<String, WebSocketService> webSocketMap) {
